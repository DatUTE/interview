...
In an interview, if asked "How does UART work?", answer in this order:

1. **Asynchronous, no clock line**: UART is an asynchronous serial protocol. There is no shared clock wire — both devices must agree on baud rate and frame format beforehand (e.g., 8N1 = 8 data bits, no parity, 1 stop bit).
2. **Frame structure**: Data is sent one frame at a time: Start bit → Data bits → (Parity, if enabled) → Stop bit(s).
3. **Hardware-to-software path on receive**: When one byte is received, the UART sets the **RXNE** (Receive Not Empty) flag. If the RX interrupt is enabled, the UART raises an IRQ to the **NVIC**, and the CPU executes `USARTx_IRQHandler()` to read and process the data.

This answer shows you understand both the physical protocol and the software interrupt path.

```c
// Typical RX interrupt handler pattern
...
In an interview, if asked "How does SPI work?", answer in this order:

1. **Synchronous master-slave, full-duplex**: SPI uses a shared clock (**SCLK**) generated by the master. The master selects one slave at a time by asserting **CS** (active low). Data goes out on **MOSI** and in on **MISO** at the same time — every clock cycle shifts one bit in both directions.
2. **Clock mode must match**: Both sides must agree on **CPOL** (clock idle level) and **CPHA** (which edge samples data). Example: Mode 0 = idle low, sample on rising edge.
3. **Hardware-to-software path**: The master writes a byte to the SPI data register; the peripheral shifts it out on MOSI while shifting in MISO. When the transfer completes, hardware sets **RXNE** (or triggers a DMA half/full complete callback). The CPU reads `DR` in `SPIx_IRQHandler()` or processes the DMA buffer — no per-bit CPU work.

This answer shows you understand clocked shifting, slave select, and how the MCU offloads bit timing to hardware.

```c
// Typical SPI TX+RX (full-duplex one byte)
uint8_t spi_transfer(SPI_TypeDef *spi, uint8_t tx) {
...
In an interview, if asked "How does I2C work?", answer in this order:

1. **Synchronous, half-duplex, multi-master capable**: I2C uses two open-drain wires — **SCL** (clock) and **SDA** (data) — with pull-up resistors. Only one device drives the bus at a time; multiple masters arbitrate by monitoring SDA while clocking.
2. **Transaction structure**: The master generates **START** (SDA falls while SCL high), sends **7-bit address + R/W bit**, waits for **ACK** (slave pulls SDA low), then transfers one or more data bytes (each followed by ACK/NACK), and finishes with **STOP** (SDA rises while SCL high).
3. **Hardware-to-software path**: The I2C peripheral state machine handles START/address/data/ACK on the wire. On events (address matched, byte received, STOP), it sets status flags and raises **EV IRQ** (and **ER IRQ** on bus errors). The CPU runs `I2Cx_EV_IRQHandler()` to read `DR`, advance the transfer, or kick off the next phase — often with DMA for bulk reads/writes.

This answer shows you understand open-drain bus behavior, addressing, and the split between bit-level hardware and byte-level firmware.

```c
// Simplified master write: START + addr(W) + byte + STOP
// (real code uses HAL or peripheral EV interrupts between phases)
HAL_I2C_Master_Transmit(&hi2c1, slave_addr << 1, &data, 1, HAL_MAX_DELAY);
```
...
In an interview, if asked "How does CAN work?", answer in this order:

1. **Multi-master, differential, message-oriented**: CAN is a broadcast bus — there is no master/slave address. Each frame carries an **identifier (ID)** that defines message meaning and **priority** (lower numerical ID = higher priority). Physical layer uses **CAN_H / CAN_L** differential signaling; **dominant (0) overwrites recessive (1)** on the bus.
2. **Arbitration and frame format**: When two nodes transmit at once, they arbitrate bit-by-bit on the ID field — the node sending recessive but reading dominant loses and backs off. A **data frame** contains SOF → ID → control → payload (0–8 bytes classic) → CRC → ACK slot → EOF. **Bit stuffing** inserts opposite bits after five identical bits so receivers stay synchronized.
3. **Hardware-to-software path**: The **CAN controller** handles TX/RX, bit timing, stuffing, CRC, and **acceptance filters** (so the MCU only sees IDs it cares about). On successful RX, the frame lands in a **FIFO** and the controller raises an interrupt; the CPU reads ID/data from mailboxes or FIFO in `CANx_RX_IRQHandler()` and dispatches to application handlers. Error counters (TEC/REC) move the node through Error Active → Error Passive → Bus Off.

This answer shows you understand broadcast semantics, non-destructive arbitration, and controller offload vs application processing.

```c
// Typical RX callback pattern (HAL-style)
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
    CAN_RxHeaderTypeDef hdr;
    uint8_t data[8];
    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &hdr, data);
    handle_can_frame(hdr.StdId, data, hdr.DLC);
}
```
...
In an interview, if asked "How does USB work?", answer in this order:

1. **Host-centric, polled bus**: USB always has one **host** (PC, hub root, or MCU in host mode). The **device never initiates** traffic — the host schedules all transfers. Communication is packet-based on **D+ / D−** (differential); speed is inferred from the device's pull-up (full speed on D+, low speed on D−).
2. **Descriptors, endpoints, and transfer types**: After attach, the host **enumerates** the device: RESET → read **Device/Configuration/Interface/Endpoint descriptors** → **SET_ADDRESS** → **SET_CONFIGURATION**. Data moves through **endpoints** (numbered pipes). **Control** (setup/config), **Bulk** (storage), **Interrupt** (HID), **Isochronous** (audio/video) differ in bandwidth guarantees and retry behavior.
3. **Hardware-to-software path (device side)**: The **USB device controller** handles low-level signaling, endpoint FIFOs, and SETUP packet decoding. On **SETUP received**, **OUT complete**, or **IN ready**, it raises an interrupt; firmware runs `USBD_XXX_IRQHandler()` or the stack callback (e.g., `CDC_Receive_FS`, `HAL_PCD_SetupStageCallback`) to handle standard requests and application data — enumeration is mostly standard request handling, then class drivers take over.

This answer shows you understand host scheduling, descriptor-driven configuration, and the device-controller interrupt model.

```c
// Device-side: SETUP and OUT often handled by stack callbacks
